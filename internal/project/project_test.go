...
func TestLoadReadsBootstrapAndEagerFiles(t *testing.T) {
	dir := t.TempDir()
	write(t, filepath.Join(dir, ".contour", "config.yaml"),
		"bootstrap: python\neager_files:\n  - AGENTS.md\n  - docs/arch.md\n")
...
func TestLoadPrefersFirstOverlayFolder(t *testing.T) {
	dir := t.TempDir()
	// Both .contour and .claude have a config; .contour comes first in the
	// recognised order, so it wins.
	write(t, filepath.Join(dir, ".contour", "config.yaml"), "bootstrap: from-contour\n")
	write(t, filepath.Join(dir, ".claude", "config.yaml"), "bootstrap: from-claude\n")

	cfg, err := Load(dir)
	if err != nil {
		t.Fatalf("Load: %v", err)
	}
	if cfg.Bootstrap != "from-contour" {
		t.Errorf("Bootstrap = %q, want from-contour (first folder wins)", cfg.Bootstrap)
	}
}

func TestLoadFindsConfigInClaudeFolder(t *testing.T) {
	dir := t.TempDir()
	write(t, filepath.Join(dir, ".claude", "config.yaml"), "bootstrap: python\n")
